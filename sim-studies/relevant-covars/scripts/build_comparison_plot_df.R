# function that takes in loo objects for all models
# Output: comparison dataframe with elpd & se diff, # high Pareto khats, MCSE, LOO-BB (=PBMA+) and PBMA weights 

build_comparison_plot_df <- function(loos_object){
  # Input: list of loo objects for each model  
  # Output: comparison dataframe with elpd (se) diff, # high Pareto khats, MCSE, LOO-BB (=PBMA+) and PBMA weights 
  ...
  #comparison_df <- loos_object |>
  #  sort(elpd_diff) |>
  #  mutate(n_high_pareto_ks =  purrr::map_dbl(purrr::map(loos_object, ~.x$diagnostics$pareto_k), ~sum(.x>0.7)))
  ...
  full_comparisons_df = merge(comparison_df, pbma_df, by=0)
