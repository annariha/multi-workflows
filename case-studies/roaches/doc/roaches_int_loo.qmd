...
#| include: false
# sample
fitpri <- modpri$sample(data = datap, refresh=0, iter_warmup = 50, iter_sampling = 50, chains=1, parallel_chains=4)
```
...
```
```{r}
input_df_test <- input_df |> 
  rowwise() |> 
  mutate(testi = list(pmap_dbl(list(zs, alpha), function(zs, alpha, ...) zs + alpha)))

head(input_df_test)

unlist(input_df_test[1,]$zs)[1]
unlist(input_df_test[1,]$testi)[1]
```
...
# add integrands for each iteration and observation

```{r}
input_df <- input_df |>
  mutate(integrands = pmap(list(zs, sigmaz, linpreds_minus_re),
                           integrand_linpred,
                           muz = 0,
...
integrand_linpred(zs = as.numeric(unlist(input_df[1,]$zs)[1]), 
                  ...
                  linpreds_minus_re = unlist(input_df[1,]$linpreds)[1])
...
# 262 values
integrand_linpred(z_vec = as.numeric(unlist(input_df[1,]$zs)), 
                  muz_j = 0,
                  sigmaz_j = input_df[1,]$sigmaz, 
