...
df_plot_manual <- df_plot |>
  ...
  select(label, elpd_diff, elpd_diff_se, loo_bb_weight_mean, divergent)
...
p1 = ggplot(df_plot_manual, aes(elpd_diff, label, color=divergent)) + 
...
p2 = ggplot(df_plot_adapted, aes(elpd_diff, label, color=divergent)) + 
  geom_pointrange(aes(xmin=elpd_diff-elpd_diff_se, xmax=elpd_diff+elpd_diff_se), fatten = .5, size = 0.8) + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") + 
  scale_color_manual(values=c("no" = "black", "yes" = "red")) + 
  # scale_shape_manual(values=c()) + 
...
p3 = ggplot(df_plot_manual_loobb_remaining, aes(elpd_diff, label, color=divergent)) + 
  geom_pointrange(aes(xmin=elpd_diff-elpd_diff_se, xmax=elpd_diff+elpd_diff_se), fatten = .5, size= 4) + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") + 
  scale_color_manual(values=c("no" = "black", "yes" = "red")) + 
  # scale_shape_manual(values=c()) + 
  xlab("$Delta widehat textrmelpd$") +
...
p4 = ggplot(df_plot_adapted_loobb_remaining, aes(elpd_diff, label, color=divergent)) + 
  geom_pointrange(aes(xmin=elpd_diff-elpd_diff_se, xmax=elpd_diff+elpd_diff_se), fatten = .5, size= 4) + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") + 
  scale_color_manual(values=c("no" = "black", "yes" = "red")) + 
  # scale_shape_manual(values=c()) + 
...
p5 <- ggplot(df_plot_manual_loobb_remaining, aes(x = loo_bb_weight_mean, y = label, col = divergent, fill = divergent)) +
  geom_point() + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") + 
  xlab("$w tiny LOO-BB$") +
...
p6 <- ggplot(df_plot_adapted_loobb_remaining, aes(x = loo_bb_weight_mean, y = label, col = divergent, fill = divergent)) +
  geom_point() + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") + 
  xlab("$w tiny LOO-BB$") +
  scale_color_manual(values=c("yes" = "red", "no" = "black")) + 
